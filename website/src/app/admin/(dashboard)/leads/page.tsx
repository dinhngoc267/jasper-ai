 ...
 * the board must never break if `activity_log` is unavailable: every lead just
 * shows "No status changes yet" and staleness falls back to `created_at`.
 */
...
export default async function AdminLeadsPage({
  ...
  const initialOpenId = Array.isArray(leadParam) ? leadParam[0] : leadParam ?? null;
  ...
  return (
    <div>
      ...
      {leads === null || leads.length === 0 ? (
        ...
        <LeadsBoard leads={leads} activity={activity} initialOpenId={initialOpenId} />
      )}
    </div>
  );
}
